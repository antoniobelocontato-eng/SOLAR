...
<title>Dobble — v35.3 (cronômetro 15s + botão silenciar)</title>
<style>
  ...
  * { box-sizing:border-box } html,body{height:100%;margin:0}
  ...
  .is-match{
    ...
    box-shadow:0 0 0 8px rgba(250,204,21,.35),0 0 24px 12px rgba(250,204,21,.45),0 0 60px 18px rgba(250,204,21,.33);
    position: relative; filter: drop-shadow(0 0 8px rgba(250,204,21,.9));
  }
  .is-match::after{
  ...
  .is-wrong{
    border-color:#60a5fa !important; position:relative;
...
<div class="wrap">
  <div class="topbar">
    ...
    <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap">
      ...
      <button class="btn" id="muteSoundBtn" style="display:none">🔇 Silenciar</button>
...
<!-- Embutir Figuras (gera novo HTML com as imagens em base64 direto pelo navegador) -->
<div class="overlay" id="embedOverlay" style="display:none;">
  <div class="modal">
    <h3 style="margin:0 0 10px 0;">Embutir figuras</h3>
    <p class="hint">Selecione <b>21 imagens</b> (PNG/JPG) na ordem dos slides 1…21 e baixe um HTML com tudo embutido.</p>
...
/* ===== Áudio (arquivo + synth fallback + botão silenciar) ===== */
...
/* ===== FIGURAS =====
   Versão simples: usa arquivos locais slide01.png…slide21.png.
   Se quiser um HTML com tudo embutido (base64), clique ⚙️ Figuras e use o gerador.
*/
const SYMBOLS = Array.from({length:21}, (_,i)=> {
  const n = String(i+1).padStart(2,"0");
  return { id:i, label:"Slide"+(i+1), img:`slide${n}.png` };
});

/* ===== Construção do baralho (q=4 => 21 símbolos, 21 cartas, 5 símbolos por carta) ===== */
const q=4;
...
/* ===== Resultados da rodada + avanço com cronômetro 15s ===== */
async function showResultsModal(roundNumber){
  ...
  contBtn.onclick=async()=>{
    ...
    // Inicia contagem de 15s na rodada seguinte
    await roomRef.child('countdown').set({
      round: nextIndex, duration: 15,
...
function updateMuteUI(){ muteSoundBtn.textContent = isMuted ? '🔊 Ativar som' : '🔇 Silenciar'; }
...
playSoundBtn.addEventListener('click', async ()=>{
  ensureAudio();
  if(isMuted){ isMuted=false; updateMuteUI(); }
  try{
    if(roundAudioEl && roundAudioEl.src){ await roundAudioEl.play(); }
    else { startSynth(); }
    playSoundBtn.style.display='none';
    muteSoundBtn.style.display='inline-block';
  }catch(e){}
});

muteSoundBtn.addEventListener('click', async ()=>{
  isMuted = !isMuted;
  updateMuteUI();
  if(isMuted){
...
let serverOffset2 = 0;
db.ref(".info/serverTimeOffset").on("value", s => { serverOffset2 = s.val() || 0; });
const nowServer2 = () => Date.now() + serverOffset2;
...
function startCountdownBadgeUI(data){
  ...
  const tick=async ()=>{
    const remainMs = Math.max(0, countEndAt - nowServer2());
...
/* ===== Embutir Figuras (gera HTML com base64 no próprio navegador) ===== */
...
genEmbedHtml.onclick = async ()=>{
  ...
  if(!files || files.length<21){ alert('Selecione as 21 imagens (PNG/JPG) na ordem.'); return; }
  embedStatus.textContent = 'Convertendo imagens…';
  const arr = await readFilesAsDataURIs(files);
  const data = Array(21).fill(0).map((_,i)=> arr[i]);
...
function buildInlineHtml(dataUris){
  const src = document.documentElement.outerHTML
    .replace(/const SYMBOLS = [\s\S]*?;\n/, 'const SYMBOLS = '+JSON.stringify(dataUris.map((d,i)=>({id:i,label:"Slide"+(i+1),img:d})))+';\n')
    .replace(/title>[\s\S]*?<\/title>/, 'title>Dobble — v35.3 (embutido)</title>');
  return '<!doctype html>\n' + src;
}

/* ===== Subs ===== */
db.ref(".info/serverTimeOffset").on("value", s => { /* já tratado acima */ });
